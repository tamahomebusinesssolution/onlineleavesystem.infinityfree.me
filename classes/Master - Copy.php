...
Class Master extends DBConnection {
	...
	function save_employee(){
        ...
        $data = "";
        foreach($_POST as $k =>$v){
            if(in_array($k,array('user_id','employee_id','firstname','lastname','middlename','username','salary','type','position_id'))){
                if(!empty($data)) $data.=" , ";
                $data .= " `{$k}` = '{$v}' ";
            }
        }
  ...
  //////////////////////////////////////////////////////////////////// 
			...
			$current_sl = $this->conn->query("SELECT l.*,concat(u.sl_current) as 'slCurrent',lt.code,lt.name as lname from `leave_applications` l inner join `users` u on l.user_id=u.id inner join `leave_types` lt on lt.id = l.leave_type_id  ")->fetch_array()['slCurrent'];

			$total_sl = $this->conn->query("SELECT l.*,concat(u.lastname,', ',u.firstname,' ',u.middlename) as `name`,concat(u.sl_total) as 'slTotal',lt.code,lt.name as lname from `leave_applications` l inner join `users` u on l.user_id=u.id inner join `leave_types` lt on lt.id = l.leave_type_id ")->fetch_array()['slTotal'];

			$current_vl = $this->conn->query("SELECT l.*,concat(u.vl_current) as 'vlCurrent',lt.code,lt.name as lname from `leave_applications` l inner join `users` u on l.user_id=u.id inner join `leave_types` lt on lt.id = l.leave_type_id  ")->fetch_array()['vlCurrent'];

			$total_vl = $this->conn->query("SELECT l.*,concat(u.lastname,', ',u.firstname,' ',u.middlename) as `name`,concat(u.vl_total) as 'vlTotal',lt.code,lt.name as lname from `leave_applications` l inner join `users` u on l.user_id=u.id inner join `leave_types` lt on lt.id = l.leave_type_id ")->fetch_array()['vlTotal'];
   ...
   //          $data .= ", `leave_days` = '{$balance}' ";	
	...
	function save_credit(){
		...
		$allowed = $this->conn->query("SELECT COALESCE(sum(`credit_days`),0) as total FROM leave_credits where user_id = '{$user_id}' and `leave_type_id` = '{$leave_type_id}' and date_format(date_start,'%Y') = '".date('Y')."' and date_format(date_end,'%Y') = '".date('Y')."' and status = 1 ")->fetch_array()['total'];
		
		$allowedCredit = $allowed + $creditDays;
		$allowable = 30 - $allowed ;

		if($leave_type_id == 4 and $creditDays > 30 ) {
	        $resp['status'] = 'failed';
			$resp['msg'] = " Service Credit is greated than 30 days allowed for a Year. Please enter {$allowable} only.";
				return json_encode($resp);
				exit;
		}
		if($creditDays == 0){
