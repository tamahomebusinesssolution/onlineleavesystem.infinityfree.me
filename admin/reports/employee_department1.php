...
<div class="card card-outline card-primary">
    ...
    <div class="card-body">
        ...
        // and `position_id` =  $position_id ;
                    $qry = $conn->query("SELECT * from `users` where `department_id` =  $department_id and `position_id` =  $position_id and `type` = 3 ");
...
<script>
    $(document).ready(function(){
        $('#filter').click(function(){
            location.replace("./?page=reports/employee_department&department_id="+($('#department_id').val())+"&position_id="+($('#position_id').val()));
        })
        $('.table').dataTable();
    })
    $(function(){
